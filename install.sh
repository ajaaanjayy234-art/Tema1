...
install_theme() {
  ...
  wget -q "$THEME_URL"
  sudo unzip -o "$(basename "$THEME_URL")" -d /root/pterodactyl || sudo unzip -o "$(basename "$THEME_URL")"
  ...
  if [ "$SELECT_THEME" -eq 1 ]; then
    ...
    sudo rm /root/C2.zip 2>/dev/null || true
    sudo rm -rf /root/pterodactyl
    ...
    yarn build:production
    php artisan view:clear
    sudo rm /root/C1.zip 2>/dev/null || true
    sudo rm -rf /root/pterodactyl
    ...
    yarn build:production
    php artisan view:clear
    sudo rm /root/C3.zip 2>/dev/null || true
    sudo rm -rf /root/pterodactyl
...
configure_wings() {
  ...
  # Minta input token dari pengguna (misal token untuk konfigurasi)
...
hackback_panel() {
  ...
  # Membuat user baru via artisan (pastikan ini sesuai kebijakan)
...
ubahpw_vps() {
  ...
  if [ "$pw" != "$pw2" ]; then
    echo -e "${RED}Password tidak cocok. Batal.${NC}"
    sleep 2
    return
  fi
...
display_welcome
install_jq
# token check removed per request
...
while true; do
  ...
  echo -e "${BLUE}'ggg      ,gg'''  .    ggg       Auto Installer 𝗦𝗔𝗚𝗔 Private   ${NC}"
  echo -e "${BLUE}gggg      gg     ,     ggg      ------------------------  ${NC}"
  echo -e "${BLUE}ggg:     gg.     -   ,ggg       • Telegram : t.me/saga011212     ${NC}"
  echo -e "${BLUE} ggg:     ggg._    _,ggg        • Creadit  : 𝗦𝗔𝗚𝗔 ${NC}"
  echo -e "${BLUE} ggg.    '.'''ggggggp           • Support by 𝗦𝗔𝗚𝗔  ${NC}"
