...
check_token() {
  ...
  if [ "$USER_TOKEN" = "sagadev" ]; then
    ...
    echo -e "${GREEN}Buy dulu Gih Ke 𝗦𝗔𝗚𝗔${NC}"
    echo -e "${YELLOW}TELEGRAM : @saga011212${NC}"
    echo -e "${YELLOW}WHATSAPP : 62895326292722${NC}"
    echo -e "${YELLOW}HARGA TOKEN : 25K FREE UPDATE JIKA ADA TOKEN BARU${NC}"
    echo -e "${YELLOW}©𝗣𝗥𝗢𝗧𝗘𝗖𝗧 𝗕𝗬 𝗦𝗔𝗚𝗔${NC}"
    exit 1
  fi
  clear
}
